...
if	IndOrPrd		== 'P':
	...
	fileLog         =  open(dirLog      + '\\matrixLmultEsa2010.log', 'w')
	fileOutput      =  open(dirOutput   + '\\matrixDBLmultEsa2010.csv', 'w')
else:
	CPA_L68			=  'L68'
	CPA_L68A		=  'L68A'
	CPA_L68B		=  'L68B'
	...
	fileLog         =  open(dirLog      + '\\matrixLmultEsa2010I.log', 'w')
	fileOutput      =  open(dirOutput   + '\\matrixDBLmultEsa2010I.csv', 'w')
...
def traitementMatrice(dicRealP1, dicMatrice, dicNoProduct):
	...
	for keyP1 in codeP1: #key = country,year
		...
		'''
		mVector       = list(map(libMatrix.divide, colEmp, colP1))
...
def readFileEMP(dicMatriceP1Row,dicGeoYearProd):
	...
	#unit,nace_r2,na_item,geo\time (nouveau ESA2010)
	#lecture fichier nama_10_a64_e
	for ligneCSV in fichierInput: 
		...
		if  (unit == 'THS_PER')and(nace_r2 != 'TOTAL')and(indic_na == 'EMP_DC'):
			for i in  range(1,len(recGeoTime)): #record des metas, on boucle sur les annees
				...
				if  keyGeoYearProd in dicGeoYearProd: #on verifie que dans chaque ligne se trouve les produits des colonnes de P1
					...
					fileLog.write('NO KEY P1 '+keyGeoYearProd+'\n')
	fichierInput.close()
...
def traitementFichierCSV():
	...
	for keyP1 in dicKeyP1Row: #key = country,year
		...
		L68_test         = 0
		entryL68EMP      = 'CPA_L68#0'
		entryL68AEMP     = 'CPA_L68A#0' 
		entryL68BEMP     = 'CPA_L68B#0'
