...
fichierCSV      =  dirInput         + '\\naio_cp18_r2.tsv'
fichierP2I      =  dirInput         + '\\naio_cp19_r2.tsv'
fileLog         =  open(dirLog      + '\\matrixMcontX.log', 'w')
fileMatrice     =  open(dirOutput   + '\\matrixMcontX.csv', 'w')
fileOutput      =  open(dirOutput   + '\\matrixMcontXAll.csv', 'w')
fileMatriceDB   =  open(dirOutput   + '\\matrixDBMcontX.csv', 'w')
...
def vectorCol(colP1,colP6):
	...
	for c in (range(0,len(colP6))):
			lineProd = colP6[c].split('#')
			lstProdP6= lstProdP6 + [lineProd[0]]            
	setcolP1         = set(lstProdP1)
	setcolP6         = set(lstProdP6)                    
	setColNotInP6    = setcolP1 - setcolP6
	lstColAll        = colP6   
	if  (len(setColNotInP6)>0):   
		for c in setColNotInP6:
			lineProd      = c + '#0'
...
def traitementMatriceA(keyP1, matriceA, dicRealP1, mVector):
	...
	colExport       = vectorCol(colP1,colP6) #la fonction fait un vecteur avec TOUTES les colonnes     
	xVector         = createExportVector(colExport)
	fileDetail      = open(dirOutput + '\\matrixMContXDetail_' + str(keyP1) +'.csv', 'w')    
	...
	#detail matrice
	'''
	fileDetail.write(str(keyP1)+ ' Matrix A '+'\n')
	ligne   = 'Code'
	l       = 0
	tabCode = []
	...
	fileDetail.write(ligne+'\n')
	ligne = ''    
	for l in range(0,nbrEle):
		ligne=tabCode[l]
		for c in range(0,nbrEle):  
			ligne   = ligne+"," + str(matriceA[l,c])     
		fileDetail.write(ligne+'\n')
	'''
...
def traitementMatrice(dicRealP1, dicMatrice, dicNoProduct):
	...
	for keyP1 in codeP1: #key = country,year
		...
		mVector       = map(divide, dicRealP1[keyP1]['P2I'], dicRealP1[keyP1]['P1'])   
...
def readFileP2I(dicMatriceP1Row):
	...
	for ligneCSV in fichierInput:
		...
		if  (unit == 'MIO_EUR')and (row == 'CPA_TOTAL')and((col[:3] == 'CPA')and(col != 'CPA_TOTAL')):
...
def traitementFichierCSV():
	...
	for ligneCSV in fichierInput:
		...
		if  (unit == 'MIO_EUR'):
			...
			if  (col == 'P6') and ((row[:3] == 'CPA')and(row != 'CPA_TOTAL')): #pas prendre cpa_total                                                                 
				for i in  range(1,len(recGeoTime)): #record des metas, on boucle sur les annees
					yy            = recGeoTime[i].strip() #record des metas
					keyGeoYY      = geo + '#' + yy
					value         = row + '#' + ligneGeoTime[i].strip()
	...
	dicMatriceP1Row = readFileP2I(dicMatriceP1Row)    
