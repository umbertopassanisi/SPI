...
if	IndOrPrd		== 'P':
	...
	fileLog         =  open(dirLog      + '\\matrixMmultEsa2010.log', 'w')
	fileOutput      =  open(dirOutput   + '\\matrixDBMmultEsa2010.csv', 'w')
else:
	CPA_L68			=  'L68'
	CPA_L68A		=  'L68A'
	CPA_L68B		=  'L68B'
	...
	fileLog         =  open(dirLog      + '\\matrixMmultEsa2010I.log', 'w')
	fileOutput      =  open(dirOutput   + '\\matrixDBMmultEsa2010I.csv', 'w')
...
def traitementMatrice(dicRealP1, dicMatrice, dicNoProduct):
	...
	for keyP1 in codeP1: #key = country,year
		...
		mVector       = map(libMatrix.divide, colP2I, colP1)    
...
def traitementFichierCSV():
	...
	for ligneCSV in fichierInput:
		...
		if  (flow == 'DOM' and unit == 'MIO_EUR'):
			...
			if	dicNace.has_key(col) and dicNace.has_key(row):
				for i in  range(1,len(recGeoTime)): #record des metas, on boucle sur les annees
					yy            = recGeoTime[i].strip() #record des metas 
