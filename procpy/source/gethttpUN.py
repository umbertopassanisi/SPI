...
def getFileUN(startYear,endYear):
	...
	for	codeNationUN in dicNation:
		...
		try:
			...
			print urlHttp
			targetFile  = dirFile + '\\xml\\'+codeNationISO2+'.xml'
			print ('start : {} at {}'.format(codeNationISO2,time.ctime()))
			try:
				#src = urllib2.urlopen(url=urlHttp,timeout=120)
				src = urllib2.urlopen(url=urlHttp)
				csv = open(targetFile,'w')
				csv.write(src.read())
				csv.close()
				print ('end : {} at {}'.format(codeNationISO2,time.ctime()))
