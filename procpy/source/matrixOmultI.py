...
fichierCSV      =  dirInput         + '\\naio_cp18i_r2.tsv'
fileLog         =  open(dirLog      + '\\matrixIOmult.log', 'w')
fileOutput      =  open(dirOutput   + '\\matrixIDBOmult.csv', 'w')
...
def traitementFichierCSV():
	...
	for ligneCSV in fichierInput:
		...
		if  (unit == 'MIO_EUR')and(geo == 'FI' or geo =='NL'):
			...
			#if  ((col != 'TOTAL') and (row != 'TOTAL')): #pas prendre cpa_total
