...
dirUse = G_path     
dirLog = dirUse + '\\Log'
fileLog = open(dirLog + '\\createDBExternalTbalRtbal' + G_nomenclature + '.log', 'w')
...
def traitementFichierTXT(nomenclature, fileLog, tableName):
    ...
    spiLibCreateTable.createTableExternalTbalRtbal(nomenclature, refDicX, refDicM, startYearM, fileLog, tableName)
