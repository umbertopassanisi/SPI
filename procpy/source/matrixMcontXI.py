...
fichierCSV      =  dirInput         + '\\naio_cp18i_r2.tsv'
fichierP2I      =  dirInput         + '\\naio_cp19i_r2.tsv'
fileLog         =  open(dirLog      + '\\matrixIMcontX.log', 'w')
fileMatrice     =  open(dirOutput   + '\\matrixIMcontX.csv', 'w')   #valeur total de indicateur
fileOutput      =  open(dirOutput   + '\\matrixIMcontXAll.csv', 'w')#details des indicateurs
fileMatriceDB   =  open(dirOutput   + '\\matrixIDBMcontX.csv', 'w') #valeurs destinees a la DB
...
def traitementFichierCSV():
	...
	for ligneCSV in fichierInput:
		...
		if  (unit == 'MIO_EUR')and (geo == 'FI' or geo =='NL'):
			...
			if  col == 'P6' and dicNace.has_key(row) and row != 'TOTAL': #pas prendre cpa_total  
				for i in  range(1,len(recGeoTime)): #record des metas, on boucle sur les annees
					yy            = recGeoTime[i].strip() #record des metas
					keyGeoYY      = geo + '#' + yy
					value         = row + '#' + ligneGeoTime[i].strip()
